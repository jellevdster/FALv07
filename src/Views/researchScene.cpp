...
//------------------------------------------------------------------
void researchScene::initResearchScene() {
...
//--------------------------------------------------------------
void researchScene::initModelEnv(){
...
void researchScene::init3DViewer(traumaType trauma){
    ...
    switch (trauma) {
        case MNH_FAL_ANTEMORTEM1:
            boneFileName = "3dmodels/Skull_321498_aka_843-reduced50k.dae";
            bonescale = 3;
            break;
    ...
    loadModel(boneFileName);
}

//------------------------------------------------------------------
void researchScene::loadModel(string filename) {
    boneModel = new ofxAssimpModelLoader();
    
    if(boneModel->loadModel(filename, true)) {
        boneModel->setAnimation(0);
        boneModel->setPosition(modelXPos, modelYPos , 0);
        boneModel->setScale(1.0, 1.0, 1.0);
        
    	mesh = boneModel->getMesh(0);
    	position = boneModel->getPosition();
    	normScale = boneModel->getNormalizedScale();
    	scale = boneModel->getScale();
    	sceneCenter = boneModel->getSceneCenter();
    	material = boneModel->getMaterialForMesh(0);
        tex = boneModel->getTextureForMesh(0);
    }
}
...
//--------------------------------------------------------------
void researchScene::removeModel() {
    delete boneModel;
    boneModel = NULL;
    bFirstRun = false;
}
...
void researchScene::touchDown(ofTouchEventArgs &touch){
    ...
    prevSpherePt = screenToSphere(ofVec2f(touch.x, touch.y));
}
...
void researchScene::touchMoved(ofTouchEventArgs &touch){
    ...
    currSpherePt = screenToSphere(ofVec3f(x, y, 0));
...
void researchScene::touchDoubleTap(ofTouchEventArgs &touch){
    ...
    if(bFirstDoubleTap) {
        boneModel->setScale(1.75, 1.75, 1.75);
    }
...
//--------------------------------------------------------------
ofVec3f researchScene::screenToSphere(ofVec3f p){
    //http://viewport3d.com/trackball.htm
    ofVec3f sphereCenter = ofVec3f(modelXPos, modelYPos);
